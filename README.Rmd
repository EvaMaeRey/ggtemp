...
The goal of {ggtemp} is to make writing some quick useful extension functions succinct. Right now, the amount of code required to do some things that I think should be more routine for analysts, is a bit of a mouthful.  Specifically, defining new geom_* and stat_* layers outside of the context of a package.  

With the {ggtemp} package, we'll live in a different world (🦄 🦄 🦄) where the task is a snap 🫰: 
